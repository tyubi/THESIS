...
const unsigned long sensorReadInterval = 300000; // 5 minutes in milliseconds (5 * 60 * 1000)
...
void setup() {
    ...
    Serial.println("Sensor readings will occur every 5 minutes");
    
    // Do an initial sensor reading at startup
    readSensors();
...
void readSensors() {
    ...
    if (distance1 < distanceThreshold1) {
        ...
        digitalWrite(ledPin1, LOW);
        Serial.println("Container 1 level: OK");
    }
    ...
    if (distance2 < distanceThreshold2) {
        ...
        digitalWrite(ledPin2, LOW);
        Serial.println("Container 2 level: OK");
    }
    
    Serial.println("Sensor reading complete");
    Serial.println("Next reading in 5 minutes");
}

void loop() {
    // Check if it's time to read sensors (every 5 minutes)
    unsigned long currentMillis = millis();
    
    // Check if 5 minutes have passed (or if millis() has overflowed)
